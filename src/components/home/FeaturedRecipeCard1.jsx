...
export default function FeaturedRecipeCard1({ recipe }) {
  const navigate = useNavigate();

  return (
    <div 
    className="group absolute top-[5%] md:top-[10%] -left-[5%] md:left-[5%] lg:left-[12%] w-36 md:w-48 lg:w-56 bg-white/80 backdrop-blur-xl border border-white p-2.5 md:p-3 rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] animate-drift-1 z-0 opacity-50 md:opacity-70 xl:opacity-100 hover:z-20 transition-all hover:scale-110 hover:shadow-[0_40px_70px_-15px_rgba(249,115,22,0.15)] hover:!animate-none duration-500 cursor-pointer"
    onClick={() => navigate(`/recipe/${recipe?._id}`)}
    >
      <div className="rounded-2xl overflow-hidden pointer-events-none">
        <img
          src={
            (recipe && recipe?.thumbnail?.secure_url) ||
            "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&q=80"
          }
          className="w-full h-24 md:h-32 object-cover transition-transform duration-700 group-hover:scale-110"
          alt={(recipe && recipe?.title) || "Poke Bowl"}
        />
      </div>
      <div className="mt-3 md:mt-4 px-2 pb-1">
        <h3 className="font-bold text-slate-800 text-xs md:text-sm tracking-tight">
          {(recipe && recipe?.title) || "Spicy Poke Bowl"}
        </h3>
        <div className="flex items-center gap-1.5 text-[10px] md:text-xs text-slate-500 mt-1 md:mt-1.5 font-medium">
          ...
          {(recipe && recipe?.averageRating) || "4.9"}{" "}
          <span className="mx-0.5">•</span>{" "}
          {(recipe && `${recipe?.totalCookingTime}m`) || "15m"}
        </div>
      </div>
    </div>
  );
